...
public class TerrenoFormViewModel : GlobalService
{
    ...
    public TerrenoFormViewModel()
    {
        ...
        ObtenerGpsCommand = new Command(async () => await ObtenerGpsAsync());
        SeleccionarMapaCommand = new Command(async () => await SeleccionarMapaAsync());
    }
    ...
    private async Task SeleccionarMapaAsync()
    {
        try
        {
            await Shell.Current.GoToAsync("//MapaSeleccionPage",
        ...
        {
            _ = MostrarToastAsync("Error al abrir mapa: " + ex.Message);
        }
    }
