...
namespace CONATRADEC.ViewModels
{
    ...
    {
        private readonly AnalisisSueloApiService analisisSueloApiService =
            new();
            ...
            try
            {
                ...
                if (terreno != null)
                {
                    ...
                    TextoBusquedaTerreno =
                        $"{terreno.CodigoTerreno} - {terreno.NombreTerreno}";
                }

                TipoCultivoResponse? cultivo =
                    TiposCultivo.FirstOrDefault(x =>
                        x.TipoCultivoId == origen.TipoCultivoId);

                if (cultivo == null)
                {
                    cultivo = contexto.TiposCultivo.FirstOrDefault(x =>
                        x.TipoCultivoId == origen.TipoCultivoId);

                    if (cultivo != null &&
                        !TiposCultivo.Any(x =>
                            x.TipoCultivoId == cultivo.TipoCultivoId))
                    {
                ...
                CantidadQuintalesOro =
                    Formatear(origen.CantidadQuintalesOro);

                TamanoFinca =
                    Formatear(origen.TamanoFinca);
                ...
                Ph = Formatear(origen.Ph);
                AcidezTotal = Formatear(origen.AcidezTotal);
                CalcioCice =
                    FormatearOpcional(origen.CalcioCice);
                MagnesioCice =
                    FormatearOpcional(origen.MagnesioCice);
                PotasioCice =
                    FormatearOpcional(origen.PotasioCice);

                AplicarMateriaOrganica(contexto, origen);
                AplicarElementos(contexto, origen);
            }
            finally
            {
            ...
            AnalisisSueloGuardarCalculoRequest origen)
        {
            ...
            materia.Valor = Formatear(origen.MateriaOrganica);
        ...
        private void AplicarElementos(
            ...
            AnalisisSueloGuardarCalculoRequest origen)
        {
            Dictionary<int, ResultadoAnalisisItemViewModel> plantillas =
                ElementosQuimicosAnalisis
                    .Where(x => x.ElementoQuimicoId.HasValue)
                    .GroupBy(x => x.ElementoQuimicoId!.Value)
                    .ToDictionary(x => x.Key, x => x.First());

            ElementosQuimicosAnalisis.Clear();

            foreach (ElementoQuimicoAnalisisRequest elemento
                     in origen.ElementosQuimicos)
            {
                if (!elemento.ElementoQuimicosId.HasValue)
                    continue;

                int id = elemento.ElementoQuimicosId.Value;

                plantillas.TryGetValue(
                    id,
                    out ResultadoAnalisisItemViewModel? plantilla);
                ...
                ElementosQuimicosAnalisis.Add(
                    new ResultadoAnalisisItemViewModel
                    {
                        ...
                        NombreParametro =
                            ...
                            ConstruirNombreElemento(catalogo, id),
                        PlaceholderValor = "Valor reportado",
                        EsConstante = false,
                        EsElementoQuimico = true,
                        PuedeEliminar = true,
                        Valor = Formatear(elemento.CantidadElemento),
        ...
        private async Task EnviarSegunModoAsync()
        {
            if (!EsModoEdicion)
            {
                if (base.EnviarAnalisisCommand.CanExecute(null))
                    base.EnviarAnalisisCommand.Execute(null);

                return;
            }

            if (IsBusy || inicializandoEdicion)
                return;
            ...
            if (!ValidarEdicion(out string error))
            {
                await MostrarAsync("Validación", error);
                return;
            }
            ...
            try
            {
                ...
                AnalisisSueloGuardarCalculoRequest request =
                    ConstruirRequestActual(contexto);

                int plantas =
                    int.TryParse(CantidadPlantas, out int cantidad)
                        ? cantidad
                        : contexto.CantidadPlantas;

                bool cambioRequerimiento =
                    AnalisisEdicionService.Instance
                        .CambioRequerimiento(request);
                ...
                if (cambioRequerimiento)
                {
                    AnalisisSueloCalculoResponse? respuesta =
                        await analisisSueloApiService.CalcularAsync(
                            CrearRequestCalcular(request));
                ...
                {
                    resultado = contexto.ResultadoOriginal;
                }

                AnalisisEdicionService.Instance.GuardarFormularioActual(
                    request,
                    plantas);
                ...
                Dictionary<string, object> parametros = new()
                {
                    ...
                    ["requestGuardarAnalisis"] = request,
                    ["cantidadPlantas"] = plantas,
                    ["esModoEdicion"] = true,
                    ["analisisSueloCalculoIdEdicion"] =
                        contexto.AnalisisSueloCalculoId,
                    ["calcularBalanceFormula"] = contexto.TieneBalance,
                    ["calcularEnmiendaCalcarea"] = contexto.TieneEnmienda,
                    ["calcularFertilizacionMixta"] = contexto.TieneMixta,
                    ["calculosRestaurados"] = true
                };
            ...
            catch (Exception ex)
            {
                await MostrarAsync(
                    "Error",
                    $"No fue posible continuar la edición: {ex.Message}");
            }
            finally
            {
                IsBusy = false;
        ...
        private AnalisisSueloGuardarCalculoRequest ConstruirRequestActual(
            AnalisisEdicionContexto contexto)
        {
            ResultadoAnalisisItemViewModel? materia =
                ParametrosConstantesAnalisis.FirstOrDefault(x =>
            ...
            return new AnalisisSueloGuardarCalculoRequest
            {
                ...
                TipoAnalisisSueloId =
                    contexto.RequestOriginal.TipoAnalisisSueloId,
                UsuarioId =
                    contexto.RequestOriginal.UsuarioId ?? UsuarioId,
                CantidadQuintalesOro =
                    Decimal(CantidadQuintalesOro),
                TamanoFinca = Decimal(TamanoFinca),
                Ph = Decimal(Ph),
                MateriaOrganica =
                    DecimalOpcional(materia?.Valor),
                UnidadMedidaMateriaOrganicaId =
                    materia?.UnidadSeleccionada?.UnidadMedidaId,
                AcidezTotal = DecimalOpcional(AcidezTotal),
                CalcioCice = DecimalOpcional(CalcioCice),
                MagnesioCice = DecimalOpcional(MagnesioCice),
                PotasioCice = DecimalOpcional(PotasioCice),
                ElementosQuimicos =
                    ElementosQuimicosAnalisis
                        .Select(x =>
                            new ElementoQuimicoAnalisisRequest
                            {
                                ElementoQuimicosId =
                                    x.ElementoQuimicoId,
                                UnidadMedidaId =
                                    x.UnidadSeleccionada?.UnidadMedidaId,
                                CantidadElemento = Decimal(x.Valor)
                            })
                        .ToList(),
                FuentesOrganicas =
        ...
        private static AnalisisSueloCalcularRequest CrearRequestCalcular(
            AnalisisSueloGuardarCalculoRequest request)
        {
            return new AnalisisSueloCalcularRequest
            {
                TerrenoId = request.TerrenoId,
                TipoCultivoId = request.TipoCultivoId,
                TipoAnalisisSueloId =
                    request.TipoAnalisisSueloId,
                UsuarioId = request.UsuarioId,
                CantidadQuintalesOro =
                    request.CantidadQuintalesOro,
                TamanoFinca = request.TamanoFinca,
                Ph = request.Ph,
                MateriaOrganica = request.MateriaOrganica,
                UnidadMedidaMateriaOrganicaId =
                    request.UnidadMedidaMateriaOrganicaId,
                AcidezTotal = request.AcidezTotal,
                CalcioCice = request.CalcioCice,
                MagnesioCice = request.MagnesioCice,
                PotasioCice = request.PotasioCice,
                ElementosQuimicos = request.ElementosQuimicos,
                FuentesOrganicas = request.FuentesOrganicas
            };
        }

        private bool ValidarEdicion(out string error)
        {
            error = string.Empty;
            ...
            else if (!Positivo(CantidadQuintalesOro))
                error = "Los quintales oro deben ser mayores que cero.";
            else if (!Positivo(TamanoFinca))
                error = "El tamaño de finca debe ser mayor que cero.";
            else if (!EnteroPositivo(CantidadPlantas))
                error = "La cantidad de plantas debe ser mayor que cero.";
            else if (!Rango(Ph, 0, 14))
                error = "El pH debe estar entre 0 y 14.";
            else if (ElementosQuimicosAnalisis.Count == 0)
                error = "Debe conservar al menos un elemento químico.";
            else if (ElementosQuimicosAnalisis.Any(x =>
                         x.ElementoQuimicoId is null or <= 0 ||
                         x.UnidadSeleccionada?.UnidadMedidaId is null or <= 0 ||
                         !NoNegativo(x.Valor)))
            {
                error =
                    "Revise el valor y la unidad de cada elemento químico.";
            }
        ...
        private async Task CancelarSegunModoAsync()
        {
            if (!EsModoEdicion)
            {
                if (base.CancelarCommand.CanExecute(null))
                    base.CancelarCommand.Execute(null);
            ...
            int id)
        {
            ...
            string nombre =
                elemento.NombreElementoQuimico ?? $"Elemento #{id}";
        ...
        private static decimal Decimal(string? valor) =>
            decimal.TryParse(
        ...
        private static bool Positivo(string? valor) =>
            Decimal(valor) > 0;

        private static bool NoNegativo(string? valor) =>
            Decimal(valor) >= 0;

        private static bool EnteroPositivo(string? valor) =>
        ...
        private static string FormatearOpcional(decimal? valor) =>
            valor.HasValue && valor.Value != 0
                ? Formatear(valor)
                : string.Empty;
