...
public class FuenteNutrienteApiService
{
            ...
            CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            if (response.IsSuccessStatusCode)
                return ResultadoFuente(contenido, "Fuente creada correctamente.");
            ...
            if (response.StatusCode == HttpStatusCode.Conflict)
            {
                ...
                {
                    return await ResolverInactivaAsync(
                        fuente,
                        conflicto.Data,
                        cancellationToken);
                }
            ...
            return ErrorFuente(response.StatusCode, contenido, "crear la fuente");
        }
        catch (Exception ex)
        {
        ...
        try
        {
            ...
            return ApiResult<bool>.Fail(
                await ApiServiceHelper.ReadResponseMessageAsync(
                    response,
                    ObtenerMensajeHttp(response.StatusCode, "eliminar la fuente"),
                    cancellationToken),
        ...
        FuenteNutrienteRequest fuente)
    {
        ...
        return resultado.Success && resultado.Data?.FuenteNutrientesId > 0;
    }

    public async Task<FuenteNutrienteResponse?>
        CreateFuenteNutrienteConRespuestaAsync(FuenteNutrienteRequest fuente)
    {
        ApiResult<FuenteNutrienteResponse> resultado =
            await CreateFuenteNutrienteResultAsync(fuente);
        return resultado.Success ? resultado.Data : null;
    ...
    private async Task<ApiResult<FuenteNutrienteResponse>> ResolverInactivaAsync(
        FuenteNutrienteRequest fuente,
    ...
    private async Task<ApiResult<FuenteNutrienteResponse>> EnviarFuenteAsync(
        HttpMethod metodo,
        string ruta,
        FuenteNutrienteRequest fuente,
        string operacion,
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            {
                Content = JsonContent.Create(fuente, options: jsonOptions)
            };
            ...
            return response.IsSuccessStatusCode
                ? ResultadoFuente(contenido, mensajeExito)
                : ErrorFuente(response.StatusCode, contenido, operacion);
        }
        catch (Exception ex)
        {
            return ErrorExcepcion<FuenteNutrienteResponse>(
        ...
        CancellationToken cancellationToken)
    {
        try
        {
            using var request = new HttpRequestMessage(metodo, ruta)
            {
                Content = JsonContent.Create(contenido, options: jsonOptions)
            };
            using HttpResponseMessage response = await httpClient.SendAsync(
                request,
                cancellationToken);
            ...
            if (response.IsSuccessStatusCode)
                return ApiResult<bool>.Ok(true, mensajeExito);

            return ApiResult<bool>.Fail(
                await ApiServiceHelper.ReadResponseMessageAsync(
                    response,
                    ObtenerMensajeHttp(response.StatusCode, operacion),
                    cancellationToken),
                (int)response.StatusCode);
        }
        catch (Exception ex)
        {
            return ErrorExcepcion<bool>(ex, cancellationToken, operacion);
        }
    }
            ...
            CancellationToken cancellationToken)
    {
        try
        {
            ...
            return ApiResult<ObservableCollection<FuenteNutrienteResponse>>.Ok(
                data ?? new ObservableCollection<FuenteNutrienteResponse>());
        }
        catch (Exception ex)
        {
        ...
        object? contenido = null)
    {
        try
        {
            using var request = new HttpRequestMessage(metodo, ruta);
            if (contenido != null)
                request.Content = JsonContent.Create(contenido, options: jsonOptions);
    ...
    private ApiResult<FuenteNutrienteResponse> ResultadoFuente(
        ...
        string mensaje)
    {
        FuenteNutrienteResponse? fuente = DeserializarFuente(contenido);

        return fuente?.FuenteNutrientesId > 0
            ? ApiResult<FuenteNutrienteResponse>.Ok(fuente, mensaje)
            : ApiResult<FuenteNutrienteResponse>.Fail(
    ...
    private ApiResult<FuenteNutrienteResponse> ErrorFuente(
        ...
        ApiResult<FuenteNutrienteResponse>.Fail(
            ApiErrorMessageParser.Parse(
                codigo,
                contenido,
                ObtenerMensajeHttp(codigo, operacion)),
            (int)codigo);

    private FuenteNutrienteResponse? DeserializarFuente(string contenido)
    {
        ...
        ApiDataResponse<FuenteNutrienteResponse>? sobre =
            DeserializarSobre(contenido);
        return sobre?.Data ?? JsonSerializer.Deserialize<FuenteNutrienteResponse>(
            contenido,
            jsonOptions);
    }

    private ApiDataResponse<FuenteNutrienteResponse>? DeserializarSobre(
        string contenido) =>
        ...
        string operacion)
    {
        string mensaje = ex switch
        {
            ...
            HttpRequestException => "No fue posible conectarse con el servidor.",
            JsonException => "La respuesta del servidor no tiene el formato esperado.",
            _ => $"Ocurrió un error inesperado al {operacion}."
        };
    ...
    private static string ObtenerMensajeHttp(
        HttpStatusCode codigo,
        string operacion) => codigo switch
    {
        HttpStatusCode.BadRequest => $"Datos inválidos al {operacion}.",
        HttpStatusCode.Unauthorized => "La sesión no está autorizada.",
        HttpStatusCode.Forbidden => $"No tiene permiso para {operacion}.",
        HttpStatusCode.NotFound => "No se encontró el recurso solicitado.",
        HttpStatusCode.Conflict => $"Existe un conflicto al {operacion}.",
        >= HttpStatusCode.InternalServerError => "El servidor presentó un problema.",
        _ => $"No fue posible {operacion}. Código HTTP: {(int)codigo}."
    };
